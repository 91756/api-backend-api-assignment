...
app.delete("/delete", async (request, response) => {
  const deleteUserQuery = `DELETE FROM users WHERE first_name = '';`;
  await db.run(deleteUserQuery);
  response.send("deleted");
});

app.delete("/", async (request, response) => {
  const getUsersQuery = `DELETE FROM users WHERE id IS null;`;
  const usersArray = await db.run(getUsersQuery);
  response.send(usersArray);
});
